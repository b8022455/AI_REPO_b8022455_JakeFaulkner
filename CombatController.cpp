...
void PlayerWeapon::PositionWeapon(std::vector<std::unique_ptr<RenderItem>> &mAllRitems)
{
	...
	mAllRitems.at(1)->Instances.at(0).MaterialIndex = 1;
	mAllRitems.at(1)->Instances.at(0).World = weaponPositionMatrix2;	///In future, change position to be where the player is	
}
...
void PlayerWeapon::SwingWeapon(std::vector<std::unique_ptr<RenderItem>> &mAllRitems)
{
	...
	else
	{
		...
		mAllRitems.at(1)->Instances.at(0).World = weaponPositionMatrix2;
	}
}
