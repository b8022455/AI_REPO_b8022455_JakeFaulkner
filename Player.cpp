...
void Player::Update(const GameTimer & gt)
{
	...
	if (mpInstance->World._41 >= GC::PLAYER_RIGHTBOUND - 0.9375f) { // 32 tiles = 30f world space, 1 tile = 0.9375f world space
		genArea = true;

	}
	if (mpInstance->World._41 <= GC::PLAYER_LEFTBOUND + 0.9375f) { // 32 tiles = 30f world space, 1 tile = 0.9375f world space
		genArea = true;

	}
	if (mpInstance->World._43 >= GC::PLAYER_UPBOUND - 0.9375f) { // 32 tiles = 30f world space, 1 tile = 0.9375f world space
		genArea = true;

	}
	if (mpInstance->World._43 <= GC::PLAYER_DOWNBOUND + 0.9375f) { // 32 tiles = 30f world space, 1 tile = 0.9375f world space
		genArea = true;

	}

	if (mpInstance->World._41 <= GC::PLAYER_RIGHTBOUND) {
		if (Slowed == true)
	...
	if (mpInstance->World._41 >= GC::PLAYER_LEFTBOUND)
	{
		if (Slowed == true)
	...
	if (mpInstance->World._43 <= GC::PLAYER_UPBOUND)
	{
		if (Slowed == true)
	...
	if (mpInstance->World._43 >= GC::PLAYER_DOWNBOUND)
	{
		if (Slowed == true)
