...
public:
	...
	void MoveEnemyFromPopulation(const int vectorOffset);		//Puts a candidate from current generation into reserved vector when they die
...
private:
	...
	int GetRandomInt(int min, int max);		//Returns a random integer value, used to get random values for initial candidates genetic information and mutations	
	...
	const int EnemyCount = 5;				//Number of enemies always present in each generation, useful for loops
};